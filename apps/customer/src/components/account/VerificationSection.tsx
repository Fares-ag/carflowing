...
export default function VerificationSection() {
  ...
  const { data: phoneStatus, isLoading: loadingPhoneStatus } = usePhoneVerificationStatus()
  ...
  const phoneBadge = phoneVerificationBadge(phoneStatus)
  const phoneDisplay = phoneVerificationDisplay(phoneStatus, phone)
        ...
        <div className="verification-item">
          <div className="verification-info">
            ...
            <div>
              ...
              <p className="verification-item-value">
                {loadingPhoneStatus ? 'Loading…' : phoneDisplay}
              </p>
            </div>
          </div>
          <span className={`status-badge ${phoneBadge.tone}`}>
            {loadingPhoneStatus ? 'Loading…' : phoneBadge.label}
          </span>
        </div>

        {!loadingPhoneStatus && phoneStatus?.smsVerificationAvailable && !phoneStatus.smsVerified ? (
          <p className="verification-phone-note">
            Verify your phone in{' '}
            <Link to="/settings?section=security" className="verification-phone-link">
              Security settings
            </Link>{' '}
            to confirm ownership and enable SMS alerts.
            {phoneStatus.smsDevFallback ? (
              <span className="verification-phone-note__dev">
                {' '}
                In development, SMS codes are logged on the server instead of being texted.
              </span>
            ) : null}
          </p>
        ) : null}

        {!loadingPhoneStatus && phoneStatus && !phoneStatus.smsVerificationAvailable ? (
          <p className="verification-phone-note verification-phone-note--muted">
